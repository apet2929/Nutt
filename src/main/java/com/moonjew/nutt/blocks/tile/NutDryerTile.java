...
public class NutDryerTile extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
    ...
    private ItemStackHandler createHandler(){
        return new ItemStackHandler(1) {
            ...
            public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
                return stack.getItem().getTags().contains("nutTag");
            }

            @Nonnull
            @Override
            public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate) {
                if(stack.getItem().getTags().contains("nutTag")){
                    return stack;
                }
    ...
    public Container createMenu(int i, PlayerInventory playerInventory, PlayerEntity playerEntity) {
        assert world != null;
        return new NutDryerContainer(i, world, pos, playerInventory, playerEntity);
    }
}